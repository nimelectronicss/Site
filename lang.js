...
const translations = {
    en: {
        ...
        calculator: { title: "📟 Simple Calculator in C", about: "📖 About", aboutText: "This is a simple command-line calculator written in C. The user enters two numbers and an operator (+, -, *, /), and the program displays the result.", source: "💻 Source Code", liveDemo: "⚡ Try it yourself (Live Simulator)", instruction: "Enter two numbers and choose an operator:", enterNum1: "First number", enterNum2: "Second number", runBtn: "▶ Run Calculation", result: "Result will appear here", note: "💡 This simulates the exact logic of the C program above.", back: "← Back to Projects" },
        atomic: { title: "⚛️ Electron Configuration", subtitle: "Enter an atomic number (1 to 118) to get the electron configuration of that element.", placeholder: "e.g., 29 (Copper)", showBtn: "🔬 Show Configuration", resultText: "Result will appear here...", note: "💡 Follows Aufbau principle, Pauli exclusion, and Hund's rule.", exceptionNote: "⚠️ Note: Exceptions (Cr, Cu, Mo, Ag, Au) are handled correctly.", back: "← Back to Projects" },
        projectsPage: {
    ...
    fa: {
        ...
        calculator: { title: "🧮 ماشین حساب ساده به زبان C", about: "📖 درباره پروژه", aboutText: "این یک ماشین حساب خط فرمان ساده است که با زبان C نوشته شده. کاربر دو عدد و یک عملگر (+, -, *, /) وارد می‌کنه و برنامه نتیجه رو محاسبه می‌کنه.", source: "💻 کد منبع", liveDemo: "⚡ خودتان امتحان کنید (شبیه‌ساز زنده)", instruction: "دو عدد و یک عملگر انتخاب کنید:", enterNum1: "عدد اول", enterNum2: "عدد دوم", runBtn: "▶ اجرا", result: "نتیجه در اینجا نمایش داده می‌شود", note: "💡 این شبیه‌ساز دقیقاً منطق برنامه C بالا را اجرا می‌کند.", back: "← بازگشت به پروژه‌ها" },
        atomic: { title: "⚛️ آرایش الکترونی", subtitle: "عدد اتمی (۱ تا ۱۱۸) را وارد کنید تا آرایش الکترونی آن عنصر را دریافت کنید.", placeholder: "مثال: ۲۹ (مس)", showBtn: "🔬 نمایش آرایش", resultText: "نتیجه در اینجا نمایش داده می‌شود...", note: "💡 پیروی از اصل آفبا، اصل پاولی و قانون هوند", exceptionNote: "⚠️ نکته: استثناها (کروم، مس، مولیبدن، نقره، طلا) به درستی اعمال شده‌اند.", back: "← بازگشت به پروژه‌ها" },
        projectsPage: {
...
function updatePageText() {
    ...
    if(document.querySelector('[data-i18n="calculator.title"]')) {
        ...
        document.querySelectorAll('[data-i18n="calculator.source"]').forEach(el => el.textContent = t.calculator.source);
        document.querySelectorAll('[data-i18n="calculator.liveDemo"]').forEach(el => el.innerHTML = t.calculator.liveDemo);
        document.querySelectorAll('[data-i18n="calculator.instruction"]').forEach(el => el.textContent = t.calculator.instruction);
        ...
        document.querySelectorAll('[data-i18n="calculator.result"]').forEach(el => el.textContent = t.calculator.result);
        document.querySelectorAll('[data-i18n="calculator.note"]').forEach(el => el.textContent = t.calculator.note);
        document.querySelectorAll('[data-i18n="calculator.back"]').forEach(el => el.textContent = t.calculator.back);
    }
    ...
    if(document.querySelector('[data-i18n="atomic.title"]')) {
        ...
        document.querySelectorAll('[data-i18n="atomic.placeholder"]').forEach(el => el.placeholder = t.atomic.placeholder);
        document.querySelectorAll('[data-i18n="atomic.showBtn"]').forEach(el => el.textContent = t.atomic.showBtn);
        document.querySelectorAll('[data-i18n="atomic.resultText"]').forEach(el => el.textContent = t.atomic.resultText);
